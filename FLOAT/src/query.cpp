...
int main(int argc, char* argv[]) {
    ...
    // ef_search_values.push_back(1000);
    // ef_search_values.push_back(1100);
    // ef_search_values.push_back(1200);
    ...
    for (int ef : ef_search_values) {
        ...
        //         // all_labels[i].push_back(int(result.top().second));
        //         result.pop();
        //     }
        // }
        std::vector<std::vector<int>> all_labels(NQ);
        ...
        summary_file << "original," << std::string(DATASET) << "," << DIM << "," << getProcessPeakRSS() << "," << 32*DIM/8 << "," << ef <<
