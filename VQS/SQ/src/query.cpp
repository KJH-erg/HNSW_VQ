...
int main(int argc, char* argv[]) {
    ...
    for (faiss::ScalarQuantizer::QuantizerType qtype : quantizer_types) {
        ...
        std::string index_path = "../../indices/SQ/" + std::to_string(DIM) + "/" +label+"_"+ std::string(DATASET) + "_hnsw.bin";
        ...
        for (int ef : ef_search_values) {
            ...
            summary_file << "SQ," << std::string(DATASET) << "," << DIM << "," << getProcessPeakRSS() << "," << std::stoi(label)*DIM/8 << "," << ef <<
