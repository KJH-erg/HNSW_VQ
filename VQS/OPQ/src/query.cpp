...
std::vector<int> get_proper_divisors(int dims) {
    std::vector<int> valid_Ms;
    for (int b = 1; b <= 16; ++b) {
        for (int m = 1; m <= dims; ++m) {
            if (dims % m == 0) {
                valid_Ms.push_back(m);  // M = # subquantizers
            }
        }
    }
    return valid_Ms;
...
int main(int argc, char* argv[]) {
    ...
    float* query_ptr = query.data();
    omp_set_num_threads(thread_num);
    
    // for (int d : divisors) {
    int d=64;
        std::string index_path = "../../indices/OPQ/" + std::to_string(DIM) + "/" +std::to_string(d)+"_"+ std::string(DATASET) + "_hnsw.bin";
        std::string opq_path = "../../indices/OPQ/" + std::to_string(DIM) + "/" +std::to_string(d)+"_"+ std::string(DATASET) + "_opq.bin";
        faiss::VectorTransform* vt = faiss::read_VectorTransform(opq_path.c_str());
        // std::cout <<"tmp:" <<vt->implem_name() << std::endl;
        // auto* opq = dynamic_cast<faiss::LinearTransform*>(vt);
        faiss::OPQMatrix* opq = dynamic_cast<faiss::OPQMatrix*>(lt);
        if (opq == nullptr) {
            std::cerr << "Error: Failed to cast VectorTransform to OPQMatrix. Check the file: " << opq_path << std::endl;
            exit(1);
        }
        std::cout << "index_path" << ":" << index_path << std::endl;
        float* rotated_q = new float[NQ * DIM];
        opq->apply_noalloc(NQ,query_ptr, rotated_q);
        ...
        for (int ef : ef_search_values) {
            ...
            for (int i = 0; i < NQ; i++) {
                std::priority_queue<std::pair<float, hnswlib::labeltype>> result = index.searchKnn(query_ptr+i+DIM,1000);
            ...
            std::cout << "OPQ," << std::string(DATASET) << "," << DIM << "," << getProcessPeakRSS() << "," << d << "," << ef <<
