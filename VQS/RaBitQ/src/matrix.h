...
public:
    ...
    Matrix(size_t n); // ID
    Matrix(char * data_file_path,bool flag);
...
template <typename T>
Matrix<T>::Matrix(char *data_file_path, bool flag){
    n = 0;
    d = 0;
    data = nullptr;
    std::cout << "opening: " << data_file_path << std::endl;
    ...
    // Read n and d (each 4 bytes = int32_t)
    int32_t n_int = 0, d_int = 0;
    in.read(reinterpret_cast<char*>(&n_int), sizeof(int32_t));
    in.read(reinterpret_cast<char*>(&d_int), sizeof(int32_t));
    n = static_cast<size_t>(n_int);
    d = static_cast<size_t>(d_int);

    std::cerr << "Number of vectors (n): " << n << std::endl;
    std::cerr << "Dimension (d): " << d << std::endl;

    // Allocate memory
    data = new T[n * d];

    // Read all data
    in.read(reinterpret_cast<char*>(data), sizeof(T) * n * d);
