...
void IVF::save(const char* filename) const {
    ...
    for (int i = 0 ; i < 100 ;i++){
        std::cout << i << ":" << static_cast<uint32_t>(IDs[i]) <<std::endl;
    }
    for (int i = 0 ; i < 100 ;i++){
        std::cout << i << ":" << static_cast<int>(SHORT_DATA[i]) <<std::endl;
    }
...
void IVF::load(const char* filename) {
    ...
    this->DQ = DataQuantizer(DIM, EX_BITS,N);
