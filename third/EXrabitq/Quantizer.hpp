...
class DataQuantizer {
   ...
   uint64_t N;
    ...
    mutable std::vector<uint8_t*> total_long_code;
    mutable std::vector<ExFactor*> total_Exfactor;
    mutable std::vector<uint8_t*> id_val_map;
    mutable std::vector<float*> id_factor_x2_map;
    mutable std::vector<uint8_t> nth_compnent_map;
   ...
   public:
    explicit DataQuantizer(size_t dim, size_t b, uint64_t N)
        : DIM(dim)
        ...
        total_long_code.assign(N,0);
        total_Exfactor.assign(N,0);
        id_val_map.assign(N,0);
        id_factor_x2_map.assign(N,0);
        nth_compnent_map.assign(N,0);
...
void DataQuantizer::quantize(
    ...
    int alinged_64 = (short_code_length()*FAST_SIZE + 63) & ~63;
    // std::cout << "short_code_length" << alinged_64<< std::endl;
    uint8_t* tmp = nullptr;
    float* cur_x2 = nullptr;
    int flag = 0;

    for (size_t i = 0; i < IDs.size(); i++) {
        // std::cout << i << std::endl;
    
        total_Exfactor[IDs[i]] = ex_factor + i;
        total_long_code[IDs[i]] = long_code + i * long_code_length();
            

            
        if (i % FAST_SIZE == 0){
            flag = 0;
            tmp = (uint8_t*) aligned_alloc(64, alinged_64);
            memset(tmp, 0, alinged_64);
            memcpy(tmp, all_short_codes + (i / FAST_SIZE)*FAST_SIZE * short_code_length(), short_code_length() * sizeof(uint8_t)*FAST_SIZE);
            cur_x2 = new float[FAST_SIZE];
            std::memcpy(cur_x2, all_factor_x2+(i / FAST_SIZE)*FAST_SIZE, sizeof(float) * FAST_SIZE);
        }
        id_val_map[IDs[i]] = tmp;
        nth_compnent_map[IDs[i]]= flag;
        id_factor_x2_map[IDs[i]] = cur_x2;
        flag++;
    }
